  ...
  const normalizeRole = (role) => {
    ...
    if (['analist', 'analista', 'analyst'].includes(compact)) return 'analista';
    if (['adm', 'admin', 'administrator'].includes(compact) || compact.startsWith('administrador')) return 'administrador';
    if (['dev', 'developer'].includes(compact)) return 'dev';
    if (['visitante', 'visitor'].includes(compact)) return 'visitante';
    return value;
  };
  ...
  const roleLabel = (role) => ({
    ...
    dev: 'Dev'
  }[normalizeRole(role)] || role || 'Visitante');
  ...
  const apiFetch = async (params, options = {}) => {
    ...
    try {
      ...
      if (json.status && json.status !== 'ok') throw new Error(json.data?.message || `Erro na API: ${json.status}`);
      return json.data;
    } finally { 
  ...
  const apiFetchJsonp = (params, _options = {}) => new Promise((resolve, reject) => {
    ...
    window[callbackName] = (payload) => {
      if (!payload || (payload.status && payload.status !== 'ok')) return cleanup(new Error(payload?.data?.message || `Erro na API: ${payload?.status || 'sem resposta'}`));
  ...
  const publicJsonpRequest = (params) => new Promise((resolve, reject) => {
    ...
    window[callbackName] = (payload) => {
      if (!payload || (payload.status && payload.status !== 'ok')) return cleanup(new Error(payload?.data?.message || `Erro na autenticação: ${payload?.status || 'sem resposta'}`));
      cleanup(null, payload.data);
    };
  ...
  const callPublicAuth = async (action, payload) => {
    try {
      ...
      const json = await res.json();
      if (json.status && json.status !== 'ok') throw new Error(json.data?.message || `Falha na autenticação: ${json.status}`);
      return json.data;
    } catch (e) {
  ...
  const saveSession = (token, user) => {
    ...
    App.currentUser = user ? { ...user, papel: normalizeRole(user.papel) } : null;
    try {
  ...
  const isPermissionError = (message) => {
    const text = normalizeAuthMessage(message);
    return text.includes('permissao insuficiente') || text.includes('permiss');
  };
  ...
  const apiPost = async (body, options = {}) => {
    ...
    const { background = false } = options;
    const requestBody = JSON.stringify({ ...body, token: App.sessionToken || '', deviceId: App.deviceId || '' });
    ...
    try {
      ...
      if (res.type === 'opaque') return { __opaqueSync: true };
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const json = await res.json();
      if (json.status && json.status !== 'ok') {
        ...
        if (isSessionExpiredError(json.data?.message)) {
          clearSession();
          showAuthShell('login');
  ...
  const hydrateAppData = (data) => {
    ...
    if (data.currentUser) App.currentUser = { ...data.currentUser, papel: normalizeRole(data.currentUser.papel) };
  ...
  const updateUserPill = () => {
    ...
    pill.style.display = '';
    pill.textContent = `${App.currentUser.nome || App.currentUser.email} • ${roleLabel(App.currentUser.papel)}`;
  };
  ...
  const renderAccess = () => {
    ...
    const hero = renderExecutiveHero(
      ...
      [
        { label: 'Perfil mais alto', value: items.find(u => normalizeRole(u.papel) === 'dev')?.nome || '—' },
    ...
    container.innerHTML = `${hero}<div class="management-grid">${items.map(user => `
      <div class="management-card">
        ...
        <div class="management-meta">
          <span class="meta-pill">${escHtml(roleLabel(user.papel))}</span>
  ...
  const updateDrawerPermissions = () => {
    const saveBtn = document.querySelector('#drawer .btn-save');
  ...
  const renderDrawerForm = (task) => {
    ...
    const comments = parseJsonSafe(t.comentarios);
    const { links } = extractBitrixLinks(t.observacoes);
    ...
    document.getElementById('drawer-body').innerHTML = `
      ...
      <div class="drawer-tab-panel${currentTab === 'overview' ? '' : ' view-hidden'}" data-tab="overview">
        ...
        <div class="form-group">
          ...
          <textarea class="form-textarea" id="f-obs" placeholder="Notas, links do Bitrix24..." ${readOnlyTask ? 'readonly' : ''}>${escHtml(t.observacoes || '')}</textarea>
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          ...
          <select class="form-select" id="u-role">
            <option value="visitante"${normalizeRole(u.papel)==='visitante'?' selected':''}>Visitante</option>
            <option value="analista"${normalizeRole(u.papel)==='analista'?' selected':''}>Analista</option>
            <option value="administrador"${normalizeRole(u.papel)==='administrador'?' selected':''}>Administrador</option>
            <option value="dev"${normalizeRole(u.papel)==='dev'?' selected':''}>Dev</option>
          </select>
        </div>
      </div>
      <div class="form-row">
  ...
  const saveTask = async () => {
    ...
    let rollbackLocalTask = null;

    try {
      if (App.currentTask) {
        payload.id           = App.currentTask.id;
        payload.data_criacao = App.currentTask.data_criacao;
        const optimisticTask = { ...App.currentTask, ...payload };
        const idx = App.tasks.findIndex(t => String(t.id) === String(App.currentTask.id));
        if (idx !== -1) {
          const previousTask = { ...App.tasks[idx] };
          rollbackLocalTask = () => {
            App.tasks[idx] = previousTask;
            App.currentTask = { ...previousTask };
          };
          App.tasks[idx] = optimisticTask;
        } else {
          rollbackLocalTask = () => {
            App.tasks = App.tasks.filter(t => String(t.id) !== String(optimisticTask.id));
            App.currentTask = null;
          };
          App.tasks.unshift(optimisticTask);
        }
        App.currentTask = optimisticTask;
        App.pendingComment = null;
        
        await updateUIAndSync('Tarefa atualizada!', 'update', payload);
      } else {
        const tempId = 'local-' + Date.now();
        App.tasks.unshift({ ...payload, id: tempId, data_criacao: new Date().toISOString() });
        rollbackLocalTask = () => {
          App.tasks = App.tasks.filter(t => t.id !== tempId);
          App.currentTask = null;
        };
        App.pendingComment = null;
        await updateUIAndSync('Tarefa criada!', 'create', payload, tempId, App.tasks);
      }
    } catch(e) {
      if (rollbackLocalTask) {
        rollbackLocalTask();
        syncAppChrome();
      }
      const message = getErrorMessage(e, 'Erro desconhecido');
      if (isPermissionError(message)) {
        toast('Erro ao salvar: a conta logada precisa estar como Analista em Acessos. O responsável escolhido na tarefa pode ser qualquer colaborador.', 'error', 7000);
      } else {
  ...
  const saveDepartment = async () => {
    ...
    try {
      if (App.currentDepartment) {
        ...
        await updateUIAndSync('Departamento atualizado!', 'updateDepartment', payload);
      } else {
        const tempId = 'dep-' + Date.now();
        App.departments.unshift({ ...payload, id: tempId, data_criacao: new Date().toISOString() });
        await updateUIAndSync('Departamento criado!', 'createDepartment', payload, tempId, App.departments);
      }
    } catch (e) {
  ...
  const saveCollaborator = async () => {
    ...
    try {
      if (App.currentCollaborator) {
        ...
        await updateUIAndSync('Colaborador atualizado!', 'updateCollaborator', payload);
      } else {
        const tempId = 'col-' + Date.now();
        App.collaborators.unshift({ ...payload, id: tempId, data_criacao: new Date().toISOString() });
        await updateUIAndSync('Colaborador criado!', 'createCollaborator', payload, tempId, App.collaborators);
      }
    } catch (e) {
  ...
  (function init() {
    ...
    try {
      ...
      if (App.currentUser) App.currentUser.papel = normalizeRole(App.currentUser.papel);
    } catch (_) {
      App.sessionToken = '';
      App.currentUser = null;
    }
