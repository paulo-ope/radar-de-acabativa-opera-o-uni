  ...
  const hasRole = (role) => (ROLE_RANK[App.currentUser?.papel] || 0) >= (ROLE_RANK[role] || 0);
  const canCreateTask = () => hasRole('administrador');
  ...
  const renderDrawerForm = (task) => {
    ...
    const pendingCommentText = App.pendingComment?.taskId === t.id ? App.pendingComment.text : '';
  ...
  const deleteCurrentTask = async () => {
    if (!canCreateTask()) return toast('Seu perfil não pode excluir tarefas.', 'warning');
